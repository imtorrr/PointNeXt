        ...
        if len(counter_part) > 1:
            for i in counter_part:
                if counter_part[i] < n or i not in parts:
                    ...
                    knn_idx = knn_point(
                        n + 1,
                        torch.unsqueeze(less_pos, axis=0),
                        torch.unsqueeze(pos[shape_idx], axis=0),
                    )[1]
                    neighbor = torch_grouping_operation(
                        pred[shape_idx : shape_idx + 1].unsqueeze(1), knn_idx
                    )[0][0]
...
def main(gpu, cfg):
    ...
    if cfg.pretrained_path is not None:
        if cfg.mode == "resume":
            ...
            test_ins_miou, test_cls_miou, test_cls_mious = validate_fn(
                model,
                val_loader,
                cfg,
            ...
            if cfg.mode in ["val", "test"]:
                ...
                logging.info(
                    f"Load encoder only, finetuning from {cfg.pretrained_path}"
                )
                load_checkpoint(
                    model_module.encoder, pretrained_path=cfg.pretrained_path
                )
    else:
    ...
    # ===> start training
    best_ins_miou, cls_miou_when_best, cls_mious_when_best = 0.0, 0.0, []
    for epoch in range(cfg.start_epoch, cfg.epochs + 1):
        ...
        if epoch % cfg.val_freq == 0:
            val_ins_miou, val_cls_miou, val_cls_mious = validate_fn(
    ...
    for idx, data in pbar:
        ...
        if idx % cfg.print_freq:
            pbar.set_description(
                f"Train Epoch [{epoch}/{cfg.epochs}] Loss {loss_meter.avg:.3f} "
            )
...
def validate(model, val_loader, cfg, num_votes=0, data_transform=None):
    ...
    cls_mious = torch.zeros(cfg.shape_classes, dtype=torch.float32).cuda(
        non_blocking=True
    )
    ...
    for idx, data in pbar:
        ...
        if cfg.get("refine", False):
            part_seg_refinement(
                preds, data["pos"], data["cls"], cfg.cls2parts, cfg.get("refine_n", 10)
            )
    ...
    ins_mious_sum, count = (
        torch.sum(torch.stack(ins_miou_list)),
        torch.tensor(len(ins_miou_list)).cuda(),
    )
    if cfg.distributed:
        (
            dist.all_reduce(cls_mious),
            dist.all_reduce(cls_nums),
            dist.all_reduce(ins_mious_sum),
    ...
    with np.printoptions(precision=2, suppress=True):
        logging.info(
            f"Test Epoch [{cfg.epoch}/{cfg.epochs}],"
